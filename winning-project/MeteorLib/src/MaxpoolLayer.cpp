...
void MaxpoolLayer::forward(const MEVectorType& inputActivation)
{
				...
				for (size_t beta = 0; beta < ih-f+1; beta+=S) 
					for (size_t alpha = 0; alpha < iw-f+1; alpha+=S)
						for (int q = 0; q < f; ++q)
							for (int p = 0; p < f; ++p)
							{
								temp1[counter].first = inputActivation[b*sizeB + r*sizeD + (beta + q)*sizeBeta + (alpha + p)].first;
								temp1[counter++].second = inputActivation[b*sizeB + r*sizeD + (beta + q)*sizeBeta + (alpha + p)].second;
							}
	}

	//Pooling operation
	if (FUNCTION_TIME){
		cout << "Meteor_funcMaxpool: " << funcTime(Meteor_funcMaxpool, temp1, activations, ow*oh*Din*B, f*f) << endl;
		//cout << "funcXNORMaxPool: " << funcTime(funcBMaxpool, temp1, activations, ow*oh*Din*B, f*f) << " milliseconds" << endl;	
	}
	else{
		//cout << "fuck xormaxpool_layer start" << endl;
		Meteor_funcMaxpool(temp1, activations, ow*oh*Din*B, f*f);
		//funcBMaxpool(temp1, activations, ow*oh*Din*B, f*f);
		//cout << "fuck maxpool layer exit!" << endl;
	}
	
	//pause_communication();
	//end_communication("maxpool" + to_string(layerNum));
	//resume_communication();
	
}


void MaxpoolLayer::computeDelta(MEVectorType& prevDelta)
{
...
void MaxpoolLayer::updateEquations(const MEVectorType& prevActivations)
{
