 ...
 outputData(LAST_LAYER_SIZE * MINI_BATCH_SIZE)
{
	for (size_t i = 0; i < NUM_LAYERS; ++i)
	{
		...
		else if (config->layerConf[i]->type.compare("Avepool") == 0) {
			AvepoolConfig *cfg = static_cast<AvepoolConfig *>(config->layerConf[i]);
			layers.push_back(new AvepoolLayer(cfg, i));
		}
