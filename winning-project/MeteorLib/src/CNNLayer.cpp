...
void CNNLayer::forward(const MEVectorType& inputActivation)
{
	...
	if (FUNCTION_TIME)
		cout << "Meteor_funcMatMul: " << funcTime(Meteor_funcMatMul, weights, temp2, temp3, Dout, (f*f*Din), (ow*oh*B), 0, 0, FLOAT_PRECISION) << " milliseconds" << endl;
	else
		Meteor_funcMatMul(weights, temp2, temp3, Dout, (f*f*Din), (ow*oh*B), 0, 0, FLOAT_PRECISION);
	...
	for (size_t i = 0; i < B; ++i)
		for (size_t j = 0; j < Dout; ++j) 
			for (size_t k = 0; k < tempSize; ++k){
				activations[i*Dout*tempSize + j*tempSize + k].first = temp3[j*B*tempSize + i*tempSize + k].first + biases[j].first;
				activations[i*Dout*tempSize + j*tempSize + k].second = temp3[j*B*tempSize + i*tempSize + k].second + biases[j].second;
			}
	//pause_communication();
	//end_communication("CNN" + to_string(layerNum));
...
void CNNLayer::computeDelta(MEVectorType& prevDelta)
{
...
void CNNLayer::updateEquations(const MEVectorType& prevActivations)
{
