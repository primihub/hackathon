...
class PsiUDF(object):
    ...
    def __call__(self, df, *args, **kwargs):
        ...
        if args[0] == "hash":
            ...
            ip = s.getsockname()[0]
            local_port = 15400 + df["hash_id"][0]
            file_name = "{}-{}".format(app_id, df["hash_id"][0])
            with open("tmp/f/{}".format(file_name), "w") as f:
                f.write("{}:{}".format(ip, local_port))
            kwargs["iterm_num"] = df.shape[0]
            count = 0
            while True:
                files = os.listdir("tmp/l/")
                if file_name in files:
                    with open(os.path.join("tmp/l/", file_name), "r") as f:
                        peer_address = f.read()
                    break
                else:
                    if count >= 10000:
...
t1 = time.time()
data = ray.data.read_csv("data/raw_data/f1.csv")
t2 = time.time()
data1 = data.map_batches(act, fn_args=("hash", "example_id", 5), batch_size=4096)
...
t4 = time.time()
print("time for read_data is {}, split data is {}, psi is {}".format(t2-t1, t3-t2, t4-t3))
data2.write_csv("data/psi_data/follower")