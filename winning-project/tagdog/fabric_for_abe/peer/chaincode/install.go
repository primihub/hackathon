...
func genChaincodeDeploymentSpec(cmd *cobra.Command, chaincodeName, chaincodeVersion string, path string) (*pb.ChaincodeDeploymentSpec, error) {
	...
	cds, err := getChaincodeDeploymentSpecByCode(spec, true, path)
	if err != nil {
...
func chaincodeInstall(cmd *cobra.Command, ccpackfile string, cf *ChaincodeCmdFactory) error {
	...
	ccpackmsg, err = genChaincodeDeploymentSpec(cmd, chaincodeName, chaincodeVersion, ccpackfile)
	if err != nil {
		return err
	}
	// } else {
