...
// DeleteHandle 删除cpabe策略处理方法
func DeleteHandle(c *gin.Context) {
	hd := &DeleteHandler{}
	handler.Handle(c, hd)
}

// DeleteHandler 删除cpabe策略处理对象
type DeleteHandler struct {
	Req  DeleteRequest
	Resp DeleteResponse
}

// DeleteRequest 删除cpabe策略请求
type DeleteRequest struct {
	Username string `json:"username"`
	ID       string `json:"id" binding:"required"`
}

// DeleteResponse 删除cpabe策略返回
...
func (h *DeleteHandler) Process() {
	// TODO: 判断是否是自己的cpabe策略

	// 调用合约删除
	resp, err := fabric.ClientInstance.Invoke("", "deletePolicy", fabric.PackArgs([]string{h.Req.ID}), nil, nil)
	if err != nil {
