...
// DeleteHandle 删除资产包处理方法
func DeleteHandle(c *gin.Context) {
	hd := &DeleteHandler{}
	handler.Handle(c, hd)
}

// DeleteHandler 删除资产包处理对象
type DeleteHandler struct {
	Req  DeleteRequest
	Resp DeleteResponse
}

// DeleteRequest 删除资产包请求
type DeleteRequest struct {
	Username string `json:"username"`
	ID       string `json:"id" binding:"required"`
}

// DeleteResponse 删除资产包返回
...
func (h *DeleteHandler) Process() {
	...
	// 调用合约删除
	resp, err := fabric.ClientInstance.Invoke("", "deleteAssetPackage", fabric.PackArgs([]string{h.Req.ID}), nil, nil)
	if err != nil {
