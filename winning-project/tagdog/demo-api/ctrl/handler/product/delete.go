...
// DeleteHandle 删除产品处理方法
func DeleteHandle(c *gin.Context) {
	hd := &DeleteHandler{}
	handler.Handle(c, hd)
}

// DeleteHandler 删除产品处理对象
type DeleteHandler struct {
	Req  DeleteRequest
	Resp DeleteResponse
}

// DeleteRequest 删除产品请求
type DeleteRequest struct {
	Username string `json:"username"`
	ID       string `json:"id" binding:"required"`
}

// DeleteResponse 删除产品返回
...
func (h *DeleteHandler) Process() {
	...
	// 调用合约删除
	resp, err := fabric.ClientInstance.Invoke("", "deleteProduct", fabric.PackArgs([]string{h.Req.ID}), nil, nil)
	if err != nil {
