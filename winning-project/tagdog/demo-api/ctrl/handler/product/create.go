...
// CreateHandle 创建产品处理方法
func CreateHandle(c *gin.Context) {
	hd := &CreateHandler{}
	handler.Handle(c, hd)
}

// CreateHandler 创建产品处理对象
type CreateHandler struct {
	Req  CreateRequest
	Resp CreateResponse
}

// CreateRequest 创建产品请求
...
func (h *CreateHandler) Process() {
	...
	if h.Req.Policy != "" {
		...
		if err != nil {
			msg := fmt.Sprintf("marshal Product error, %v", err)
	...
	// 调用合约存证
	resp, err := fabric.ClientInstance.Invoke("", "createProduct", fabric.PackArgs([]string{cpabeEncrypted, value, h.Req.AssetPackageID, strconv.Itoa(h.Req.Amount)}), nil, nil)
	if err != nil {
