...
// CreateHandle 创建资产处理方法
func CreateHandle(c *gin.Context) {
	hd := &CreateHandler{}
	handler.Handle(c, hd)
}

// CreateHandler 创建资产处理对象
type CreateHandler struct {
	Req  CreateRequest
	Resp CreateResponse
}

// CreateRequest 创建资产请求
...
func (h *CreateHandler) Process() {
	...
	// 加密
	if err := crypto.ABEEncrypt([]string{h.Req.Username}, asset, kpAttributes, params, h.Req.Policy); err != nil {
	...
	// 调用合约存证
	resp, err := fabric.ClientInstance.Invoke("", "createAsset", fabric.PackArgs([]string{cpabeEncrypted, value, h.Req.AssetPackageID}), nil, nil)
	if err != nil {
