...
public:
  ...
  virtual bool compute_optimal_parameters() {
    ...
    */
    ...
    while (k < 1000.0) {
      const double numerator = (-k * projected_element_count); // -kn/
...
public:
  bloom_filter()
      : salt_count_(0), table_size_(0), projected_element_count_(0),
        inserted_element_count_(0), random_seed_(0),
        desired_false_positive_probability_(0.0) {}

  bloom_filter(const bloom_parameters &p)
      : projected_element_count_(p.projected_element_count),
        inserted_element_count_(0), // 0b10100101...
  ...
  // deserialize, get/load out the class member from buf
  bloom_filter(const uint8_t *buf) {
    bf_hdr *p_head = (bf_hdr *)buf;
    this->salt_count_ = p_head->salt_count_;
    this->table_size_ = p_head->table_size_;
    this->projected_element_count_ = p_head->projected_element_count_;
    this->inserted_element_count_ = p_head->inserted_element_count_;
    this->random_seed_ = p_head->random_seed_;
    this->desired_false_positive_probability_ =
        p_head->desired_false_positive_probability_;
    // std::cout << (p_head + 1) << ' ' << p_head << std::endl;
    unsigned int *salt_begin = (unsigned int *)(p_head + 1);
    unsigned int *salt_end = salt_begin + this->salt_count_;
    this->salt_ = std::vector<unsigned int>(salt_begin, salt_end);

    unsigned char *bt_begin = (unsigned char *)salt_end;
    unsigned char *bt_end = bt_begin + this->table_size_ / 8;
    this->bit_table_ = std::vector<unsigned char>(bt_begin, bt_end);
  }

  size_t compute_serialization_size() {
    // std::cout << this->bit_table_.size() << ' ' << this->table_size_ <<
    // std::endl; std::cout << this->salt_.size() << ' ' << this->salt_count_ <<
    // std::endl;
    size_t head_len = sizeof(bf_hdr); // 5*8+4 ?
    // std::cout << " sizeof header" << head_len << std::endl;
    size_t data_len = this->salt_.size() * sizeof(unsigned int) +
                      this->bit_table_.size() * sizeof(unsigned char);
    return head_len + data_len;
  }

  // put serialized bf class member into buf
  void serialize(uint8_t *buf) // size of buf: compute_serialization_size()
  {
    bf_hdr *p_head = (bf_hdr *)buf;
    p_head->salt_count_ = this->salt_count_;
    p_head->table_size_ = this->table_size_;
    p_head->projected_element_count_ = this->projected_element_count_;
    p_head->inserted_element_count_ = this->inserted_element_count_;
    p_head->random_seed_ = this->random_seed_;
    p_head->desired_false_positive_probability_ =
        this->desired_false_positive_probability_;

    unsigned int *salt_begin = (unsigned int *)(p_head + 1);
    std::memcpy(salt_begin, this->salt_.data(),
                sizeof(unsigned int) * this->salt_.size());

    unsigned char *bt_begin =
        (unsigned char *)(salt_begin + this->salt_.size());
    std::memcpy(bt_begin, this->bit_table_.data(),
                sizeof(unsigned char) * this->bit_table_.size());
  }
  ...
  template <typename T> // int, ok, int[] ok ?, int* ok ?
...
public:
  ...
  unsigned int salt_count_;              // k
  unsigned long long int table_size_;    // m / 8
  unsigned long long int projected_element_count_;
  unsigned long long int inserted_element_count_;
  unsigned long long int random_seed_;
  double desired_false_positive_probability_;
...
public:
  ...
  inline unsigned long long int size() const { return size_list.back(); }
