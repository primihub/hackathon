...
public:
  ...
  CSVWriter &add(std::string str) {
    ...
    if (foundQuotationMarks) {
      ...
      // if seperator was found and string was not escapted before, surround
      // string with "
      str = "\"" + str + "\"";
    }
  ...
  template <typename T> CSVWriter &add(T str) {
    if (this->columnNum > -1) {
  ...
  template <typename T> CSVWriter &operator<<(const T &t) {
    return this->add(t);
  }

  void operator+=(CSVWriter &csv) { this->ss << std::endl << csv; }
  ...
  void disableAutoNewRow() { this->columnNum = -1; }
  // you can use this reset method in destructor if you gonna use it in heap
  // mem.
  void resetContent() {
